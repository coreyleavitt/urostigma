...
- [ ] 10 CI: image publish (tag workflow self-gated; docker run + dig/HTTP probe before compose repoint) (in progress: delegated agent authors Dockerfile.sovereign + publish workflows + local image run-proof; control loop pushes, watches publish green, then scratch-pull verification + compose repoint)
